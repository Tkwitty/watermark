...
def plot_losscurve(lossdicts, sav):
    ...
    plt.legend()
    plt.savefig("./datas/zcylogo/" + "loss_" + sav + ".png")
    plt.show()
    print("plot over")


# D:\_Git_\watermark\logo_regression\datas
base_ds = 'D:/_Git_/watermark/logo_regression/datas'
def train_logo_zcy():
    # 准备一波样本，x为带水印的mat，y为不带水印的mat
    dpath = "nologo"  # 数据集 dir
    lw, lh = 291, 87
    xs, ys = load_logoDataset('{}/{}'.format(base_ds, dpath), (lw, lh))  # 从图片dir加载数据集
    ...
    initial_rgb = tf.fill([lh, lw, 3], 0.7)
    initial_a = tf.fill([lh, lw, 1], 0.4)
    ...
    factor2 = tf.subtract(tf.ones([lh, lw, 1], tf.float32), W_a, name=None)   # 减法
    ...
    with tf.Session() as sess:
        ...
        writer = tf.summary.FileWriter('./graphs/zcy_logo_reg', sess.graph)
    ...
    def cvt2Img(W_rgb, W_a, no):
        ...
        # irgb.save("zlogo_" + tr_version + ".png")
        irgb.save("./datas/zcylogo/" + "no.{}_zlogo_{}.png".format(no, tr_version))

    for i, iw in enumerate(train_ws):
        wrgb, wa = tuple(iw)
        cvt2Img(wrgb, wa, i)
